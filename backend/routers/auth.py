...
def register_college(
    ...
    """
    [SECURITY] Invite-token protected college registration.
    Requires a valid, unused, non-expired invite token.
    Tokens are generated by super_admin via POST /admin/create-invite.
    """
    from models import CollegeInvite
    from datetime import datetime

    # ── SECURITY GATE: validate invite token ─────────────────────────
    if not payload.invite_token:
        raise HTTPException(
            status_code=403,
            detail="An invite token is required to register a college. "                   "Please contact the system administrator."
        )

    invite = db.query(CollegeInvite).filter_by(token=payload.invite_token).first()
    ...
    if invite.expires_at < datetime.utcnow():
        raise HTTPException(status_code=403, detail="This invite has expired.")
